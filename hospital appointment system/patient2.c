...
#define PORT "21070" // the port client will be connecting to
...
//from description checking!!!!!!!!!!
    ...
    printf("Phase 1: Patient 2 has TCP port number %d and IP address ", ntohs(my_addr.sin_port));
//BJ P73
...
//read patient1.txt and send
  file_1=fopen("patient2.txt","r");
  fscanf(file_1,"%s",patient2[0]);
  fscanf(file_1,"%s",patient2[1]);
  fclose(file_1);
  strcat(head,patient2[0]);
  strcat(head," ");
  strcat(head,patient2[1]);
  if (send(sockfd, head, 50, 0) == -1)
    perror("send");
  else
    printf("Phase 1: Authentication requst from Patient 2 with username %s and password %s has been sent to the Health Center Server\n",patient2[0],patient2[1]);
...
//print the table;receive patient key board input; send number
  ...
  //  if (argc != 2) {
   ...
   // }
    ...
    //read patient1insurance.txt
    file_3=fopen("patient2insurance.txt","r");
    ...
    printf("Phase 3: Patient 2 has a dynamic UDP port %d and IP address %s\n",ntohs(my_addr_1.sin_port), inet_ntoa(*addr_list_1[0]));
    // get the Ip address of doc

    addr_len_1=sizeof(struct sockaddr);
    printf("Phase 3: The cost estimation request from Patient 2 with insurance plan %s has been sent to the doctor with port number %s and IP adress ", patient1insurance, docport);
    // receive price
