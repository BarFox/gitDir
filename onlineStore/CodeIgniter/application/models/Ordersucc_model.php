<?php
    ...
    class ordersucc_model extends CI_Model{
        ...
        public function updatecustomer($username,$customername,$customeraddress,$creditcard,$securitycode,$expirationdate,$id){
            $sql4="update customer set username=?,customername=?,customeraddress=?,creditcard=?,securitycode=?,expirationdate=? where customerid=?";
            $res4=$this->db->query($sql4, array($username,$customername,$customeraddress,$creditcard,$securitycode,$expirationdate,$id));
          //  $res4=$res4->result_array();
          //  $res4=mysql_query($sql4);
            return $res4;
        }
        public function insertorderhis($nowtime,$customeraddress,$creditcard,$id){
            $sql6="insert into orderhis (orderdate,customeraddress,creditcard,customerid) values (?,?,?,?)";
            $res6=$this->db->query($sql6, array($nowtime,$customeraddress,$creditcard,$id));
         //   $res6=$res6->result_array();
