...
sys_read(read_args_t *arg)
{
    ...
    */
    ...
    int nbytes = do_read(kern_args.fd, temp_buf, kern_args.nbytes);
    
    if(nbytes < 0) {
        dbg(DBG_PRINT, "(GRADING3D 2)\n");
        page_free(temp_buf);
        curthr->kt_errno = -nbytes;
        return -1; 
    } 

   /* err = */copy_to_user(kern_args.buf, temp_buf, nbytes);
    /*
    if(err < 0){
        KASSERT(0);
        page_free(temp_buf);
...
{		
    ...
    err = */copy_from_user(&kern_args, arg, sizeof(write_args_t));
    /*
    if (err < 0) {
        KASSERT(0);
        curthr->kt_errno = -err;
    ...
    */
   /* err = */copy_from_user(temp_buf, kern_args.buf, kern_args.nbytes);
    /*
    if (err < 0) {
        KASSERT(0);
        page_free(temp_buf);
    ...
    */
    int nbytes = do_write(kern_args.fd, temp_buf, kern_args.nbytes);
...
{	
    ...
    */
    ...
    temp_dirp = (dirent_t*)kmalloc(sizeof(dirent_t));

    while(r_count <= kern_args.count-sizeof(dirent_t)){
        dbg(DBG_PRINT, "(GRADING3B 1)\n");
    	err = do_getdent(kern_args.fd, temp_dirp);
    	if(err < 0){
            dbg(DBG_PRINT, "(GRADING3D 2)\n");
        	kfree(temp_dirp);
        	curthr->kt_errno = -err;
            ...
            dbg(DBG_PRINT, "(GRADING3B 1)\n");
            kfree(temp_dirp);
            curthr->kt_errno = -ENOENT;
            return 0;             
        }

        err = copy_to_user(kern_args.dirp+r_count, temp_dirp, sizeof(dirent_t));
        r_count+=sizeof(dirent_t);
    }
