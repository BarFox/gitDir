...
{       
    ...
    vmarea_t *vma_fault = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr));

    if(vma_fault == NULL){    
        dbg(DBG_PRINT, "(GRADING3D 2)\n");
        return 0;
    }else if((vma_fault->vma_prot & perm) == perm){
        dbg(DBG_PRINT, "(GRADING3B 1)\n");
        return 1;       
    }else{
        dbg(DBG_PRINT, "(GRADING3D 2)\n");
...
{   
    dbg(DBG_PRINT, "(GRADING3B 1)\n");
    uintptr_t temp_addr;
    temp_addr = (uintptr_t)avaddr;

    while(temp_addr <= (uintptr_t)avaddr+len){
        dbg(DBG_PRINT, "(GRADING3B 1)\n");
        temp_addr = temp_addr & 0XFFFFF000; 
        int ret = addr_perm(p, (const void *)temp_addr, perm);
        if(ret==0){
            dbg(DBG_PRINT, "(GRADING3D 2)\n");
            return 0;
        }
        temp_addr = temp_addr + 0X1000; 
    }
    return 1;
